...
struct Phase4ClippingVisualTest {
    backend: WgpuBackend,
    window: Window,
    scene: Scene,
}

impl Application for Phase4ClippingVisualTest {
    fn new(event_loop: &EventLoop) -> Self {
        println!("=== Phase 4 Clipping Visual Test (Desktop) ===");
        println!("Rendering live native scene for nested clip stack behavior.");

        let config = WindowConfig {
            title: "Phase 4 Visual Test - Clipping".to_string(),
        ...
        let scene = phase4_visual_scenes::build_phase4_clipping_scene();
...
fn main() {
    env_logger::init();
    plat_core::run::<Phase4ClippingVisualTest>()
        .expect("Failed to run Phase 4 clipping visual test");
}
