...
struct Phase4BlurVisualTest {
    backend: WgpuBackend,
    window: Window,
    scene: Scene,
}

impl Application for Phase4BlurVisualTest {
    fn new(event_loop: &EventLoop) -> Self {
        println!("=== Phase 4 Blur Visual Test (Desktop) ===");
        println!("Rendering live native scene for blur and backdrop-blur effects.");

        let config = WindowConfig {
            title: "Phase 4 Visual Test - Blur".to_string(),
        ...
        let scene = phase4_visual_scenes::build_phase4_blur_scene();
...
fn main() {
    env_logger::init();
    plat_core::run::<Phase4BlurVisualTest>().expect("Failed to run Phase 4 blur visual test");
}
