...
**[Performance] Eliminate .clone() during tree deletion by taking ownership**
**Learning:** Functions that recursively delete tree nodes (like `A11yTree::remove_node`) often `.clone()` the node to iterate its children without holding a borrow on the tree's node map. This creates expensive full-node heap allocations for every deleted node. Furthermore, iterating the children and recursively removing them triggers `O(N)` updates (`retain`) to the current node's `children` array, which is wasted work since the current node is being destroyed anyway.
**Action:** Instead of cloning, use `.remove(&id)` to extract the node and take ownership of it immediately. This eliminates the `.clone()`, and automatically gracefully skips the `retain` step for children (since the parent is no longer in the map during the recursive child deletion calls), dropping the array deletion complexity from O(N^2) to O(1).
**[Performance] Remove Vec allocations in instance collector**\n**Learning:** In 2D rendering backends, using `thread_local!` with `RefCell<Vec<Arc<Resource>>>` to avoid per-frame allocations delays the Drop of GPU resources, leading to VRAM leaks and OOMs. Instead, pre-allocating vectors natively with `Vec::with_capacity(len)` offers zero-cost allocation overhead while immediately dropping resources when they fall out of scope.\n**Action:** In hot loops processing geometry or handles, use `Vec::with_capacity(n)` derived from the input iterable length rather than `thread_local!` caches to ensure safe, deterministic resource lifetimes without the panic risks of `RefCell`.
