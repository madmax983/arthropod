...
pub fn layout_system(
    ...
    // ⚡ Bolt: Mapping style.0 avoids expensive .clone() heap operations per frame since FlexStyle is now Copy.
    layout_styles.clear();
    layout_styles.extend(query.iter().map(|(node_ref, style)| (node_ref.0, style.0)));
    ...
    let root = scene.root();
    let constraints = constraints_res.map(|r| r.0).unwrap_or_default();
