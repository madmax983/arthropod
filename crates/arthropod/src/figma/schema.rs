...
pub fn import_figma_document(json: &str) -> Result<ImportedFigmaDocument, FigmaImportError> {
    ...
    let mut ctx = ResolveContext {
        ...
        scene: &mut scene,
        root,
        figma_to_scene: &mut figma_to_scene,
        layout_styles: &mut layout_styles,
        constraints_map: &mut constraints_map,
        components: &mut components,
        instances: &mut instances,
        variant_properties: &mut variant_properties,
        component_property_definitions: &mut component_property_definitions,
        instance_property_overrides: &mut instance_property_overrides,
    };
    resolve_figma_nodes(&mut ctx);

    let prototype_edges = extract_prototype_edges(&document, &figma_to_scene);

    let resolved_instance_properties = resolve_instance_properties(
        &instances,
        &figma_to_scene,
...
struct ResolveContext<'a> {
    document: &'a FigmaDocument,
    scene: &'a mut Scene,
    root: NodeId,
    figma_to_scene: &'a mut HashMap<String, NodeId>,
    layout_styles: &'a mut HashMap<NodeId, FlexStyle>,
    constraints_map: &'a mut HashMap<NodeId, ImportedConstraints>,
    components: &'a mut HashMap<NodeId, ImportedComponentNode>,
    instances: &'a mut HashMap<NodeId, ImportedInstanceNode>,
    variant_properties: &'a mut HashMap<NodeId, HashMap<String, String>>,
    component_property_definitions:
        &'a mut HashMap<NodeId, HashMap<String, ImportedComponentPropertyDefinition>>,
    instance_property_overrides:
        &'a mut HashMap<NodeId, HashMap<String, ImportedComponentPropertyOverride>>,
}

fn resolve_figma_nodes(ctx: &mut ResolveContext<'_>) {
    let mut pending: Vec<usize> = (0..ctx.document.nodes.len()).collect();
    ...
    while !pending.is_empty() {
        ...
        for index in pending {
            ...
            let parent = match &node.parent_id {
                Some(parent_id) => ctx.figma_to_scene.get(&parent_id.as_key()).copied(),
                None => Some(ctx.root),
            };
...
fn resolve_instance_properties(
    instances: &HashMap<NodeId, ImportedInstanceNode>,
    figma_to_scene: &HashMap<String, NodeId>,
    component_property_definitions: &HashMap<
        NodeId,
        HashMap<String, ImportedComponentPropertyDefinition>,
    >,
    variant_properties: &HashMap<NodeId, HashMap<String, String>>,
    instance_property_overrides: &HashMap<
        NodeId,
