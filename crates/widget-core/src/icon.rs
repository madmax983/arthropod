...
impl Icon {
    ...
    pub fn search() -> Self { Self::new("\u{e8b6}") }
    pub fn home() -> Self { Self::new("\u{e88a}") }
    pub fn settings() -> Self { Self::new("\u{e8b8}") }
    pub fn check() -> Self { Self::new("\u{e5ca}") }
    pub fn close() -> Self { Self::new("\u{e5cd}") }
    pub fn menu() -> Self { Self::new("\u{e5d2}") }
}
            ...
            NodeContent::Styled {
                style: Box::new(
                    render_engine::VisualStyle::new()
                        .solid_fill(color)
                        .text(render_engine::TextContent::new(self.icon.clone(), self.size)),
                ),
            },
        );
