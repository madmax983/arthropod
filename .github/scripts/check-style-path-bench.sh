...
BENCH_NAME="${BENCH_NAME:-path_boolean_ops/intersect/128}"
MAX_NS="${MAX_NS:-50000}"
OUTPUT_FILE="${OUTPUT_FILE:-style_path_bench_output.txt}"

echo "Running style-engine benchmark guard for: ${BENCH_NAME}"
echo "Threshold: ${MAX_NS} ns (median)"

cargo bench -p style-engine --bench path_ops -- "^${BENCH_NAME}\$" | tee "${OUTPUT_FILE}"
